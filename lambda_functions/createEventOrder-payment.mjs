...
export const handler = async (event) => {
  ...
  try {
    ...
    const userId = event.requestContext?.authorizer?.jwt?.claims?.email || studentEmail || event.requestContext?.authorizer?.jwt?.claims?.sub;
    console.log("Derived userId (email prioritized):", userId);
