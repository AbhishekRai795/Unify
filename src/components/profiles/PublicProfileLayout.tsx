  ...
  return (
    <div ref={profileRef} className={`min-h-screen transition-colors duration-300 ${isDark ? 'bg-dark-bg' : 'bg-[#fcfdfd]'}`}>
      <div className="relative h-[520px] w-full overflow-hidden bg-slate-200">
        ...
        <div className="absolute top-8 left-6 right-6 z-20 no-capture flex items-center justify-between">
          <motion.button
            onClick={onClose}
            whileHover={{ x: -4, scale: 1.02 }}
            whileTap={{ scale: 0.96 }}
            className={`px-5 py-3 backdrop-blur-xl rounded-xl transition-all border flex items-center gap-2 group ${isDark ? 'bg-dark-surface/90 border-dark-border/70 text-white hover:bg-dark-surface' : 'bg-white/60 border-white/40 shadow-md shadow-slate-200/10 text-slate-800 hover:bg-white/80'}`}
          >
            <ArrowLeft className="h-5 w-5 group-hover:-translate-x-1 transition-transform" />
            <span className="font-bold text-sm">{closeLabel}</span>
          </motion.button>
        ...
        <div className="absolute bottom-20 left-0 right-0 px-6 md:px-12">
          <div className="max-w-[1600px] mx-auto">
            <motion.div variants={heroGroup} initial="hidden" animate="visible" className="space-y-6">
              ...
              <motion.h1 variants={heroItem} className={`text-6xl md:text-9xl font-black tracking-tighter leading-none ${isDark ? 'text-white' : 'text-slate-900'}`}>
                {title}
              </motion.h1>

              <motion.div variants={heroItem} className="flex flex-col md:flex-row md:items-center gap-8 pt-4">
                <div className={`flex flex-wrap gap-8 font-bold opacity-90 ${isDark ? 'text-dark-text-secondary' : 'text-slate-500'}`}>{metaContent}</div>

                <motion.button
                  onClick={onCtaClick}
                  whileHover={{ y: -2, scale: 1.03 }}
                  whileTap={{ scale: 0.96 }}
                  className={`px-10 py-4 rounded-xl font-black text-sm shadow-md hover:scale-105 transition-all w-fit no-capture text-white ${isDark ? 'bg-accent-600 hover:bg-accent-700 shadow-accent-900/20' : 'bg-blue-600 hover:bg-blue-700 shadow-blue-500/20'}`}
                >
                  {ctaLabel}
                </motion.button>
              </motion.div>
      ...
      <div className="max-w-[1600px] mx-auto px-4 md:px-6 lg:px-8 py-12 -mt-12 relative z-10">
        ...
        <div className={`mt-24 pt-16 border-t flex flex-col md:flex-row justify-between items-center gap-8 no-capture ${isDark ? 'border-dark-border/30' : 'border-slate-200'}`}>
          <div className="flex flex-col md:flex-row items-center gap-4 md:gap-4">
            <div className={`flex items-center space-x-2 pr-4 md:border-r ${isDark ? 'border-dark-border/40' : 'border-slate-200'}`}>
