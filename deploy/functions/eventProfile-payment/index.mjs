...
export const handler = async (event) => {
  ...
  try {
    ...
    if (method === 'PUT' && (rawPath.includes('/profile') || routeKey.includes('/profile'))) {
      ...
      const chunksToEmbed = [];
      if (profilePayload.about) chunksToEmbed.push({ id: 'about', text: `Event About: ${profilePayload.about}` });
      if (profilePayload.mission) chunksToEmbed.push({ id: 'mission', text: `Event Mission: ${profilePayload.mission}` });
      if (profilePayload.vision) chunksToEmbed.push({ id: 'vision', text: `Event Vision: ${profilePayload.vision}` });
      if (profilePayload.eventDetails) chunksToEmbed.push({ id: 'details', text: `Event Details: ${profilePayload.eventDetails}` });
      if (profilePayload.highlights.length > 0) chunksToEmbed.push({ id: 'highlights', text: `Event Highlights: ${profilePayload.highlights.join('; ')}` });
      if (profilePayload.achievements.length > 0) chunksToEmbed.push({ id: 'achievements', text: `Event Achievements: ${profilePayload.achievements.join('; ')}` });
      ...
      if (chunksToEmbed.length > 0) {
        ...
        if (apiKey) {
          ...
          if (vectors && vectors.length === chunksToEmbed.length) {
            ...
            // BatchWrite supports up to 25 items, we have max ~6
            await ddb.send(new BatchWriteCommand({
              RequestItems: {
                [VECTORS_TABLE]: putRequests
              }
            }));
            console.log(`Stored ${putRequests.length} vectors for event ${eventId}`);
          }
        }
      }
