...
export const generateReceiptId = (chapterId, userId) => {
  ...
  // Razorpay limits receipt length to 40 characters (e.g. RCP-12345678-12345678-123456)
  const shortChapter = chapterId.substring(0, 8);
  const shortUser = userId.substring(0, 8);
  return `RCP-${shortChapter}-${shortUser}-${timestamp.toString().slice(-6)}`;
};
