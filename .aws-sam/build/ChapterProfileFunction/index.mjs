...
const getChapterProfile = async (chapterId) => {
  const resp = await ddb.send(new GetCommand({
...
export const handler = async (event) => {
  console.log('chapterProfile lambda event:', JSON.stringify(event, null, 2));
  ...
  if (!chapterId) {
    return { statusCode: 400, headers: corsHeaders, body: JSON.stringify({ error: 'chapterId path parameter required' }) };
  }
  ...
  try {
    ...
    if (!chapterHead || !chapterHead.chapterId || chapterHead.chapterId !== chapterId) {
      console.warn(`Blocked: Unauthorized for chapter ${chapterId}. Head email: ${email}`);
      return { statusCode: 403, headers: corsHeaders, body: JSON.stringify({ error: 'Forbidden: Unauthorized for this chapter' }) };
    }
    ...
    if (method === 'POST' && (rawPath.includes('upload-url') || routeKey.includes('upload-url'))) {
      ...
      const uniqueName = `profile-${chapterId}-${Date.now()}.${fileExtension}`;
      const s3Key = `chapters/${chapterId}/${uniqueName}`;
    ...
    if (method === 'PUT' && (rawPath.includes('/profile') || routeKey.includes('/profile'))) {
      ...
      return {
        statusCode: 200,
        headers: corsHeaders,
        body: JSON.stringify({ message: 'Chapter profile saved', profile: profilePayload })
      };
    }
